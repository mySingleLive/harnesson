...
| 2 | 引用完整性 | `design` 指向的文件是否存在；`sourceFiles` 路径是否有效 | design 指向的文件缺失 → 标记"更新"（待全量同步修复）；`sourceFiles` 路径无效 → 从 sourceFiles 移除 |
...
- `specDetail`：从代码中提取可测试、可量化的规格参数。
  ...
  - `parameters`：string 数组，每条一个可量化的规格。至少覆盖以下维度各 1 条（如功能涉及）：UI 规格（尺寸、颜色、字体、布局）、数据限制（最大值、最小值、长度限制）、行为规格（超时时间、重试次数、并发限制）。若某个维度不适用（如纯后端逻辑无 UI），在 `description` 中说明。
  - 叶子节点（`isLeaf: true` + 有 `sourceFiles`）必须生成。中间节点可选。
- `constraints`：string 数组，描述功能的边界条件和约束。
  - 至少覆盖：1 个适用场景、1 个不适用场景、1 个错误条件。此外可包含前置条件、后置条件、不变条件。
...
3a. 按以下子步骤为每个节点生成设计文档：
   ...
   e. **补全操作（仅增量）**：同步计划中标记为"补全"的节点，只生成缺失字段并合并到现有 JSON 和 design 文档，不重写整个文件。
      - JSON：从 `.harnesson/specs/` 正式目录读取现有文件，处理缺失字段后写入 `draft/`——若字段完全不存在则新增，若已存在则保留不变。数组字段（如 `parameters`、`constraints`）按条目比对：新条目追加到现有数组末尾，已有条目不变
      - Design：从 `.harnesson/specs/design/` 正式目录读取现有设计文档，若缺失 "Specification Details" 或 "Constraints" 章节，在副本中追加后写入 `draft/design/`；已存在的章节不重写
...
在展示给用户之前，先对每个草稿节点执行以下 4 项覆盖度检查。发现问题则修改草稿并重新校验（回到 Step 4），循环直到全部通过。
...
**循环规则：** 发现任一问题 → 修改草稿或添加新节点 → 回到 Step 4 重新校验 → 再回到本步重新检查 → 直到 4 项全部通过。
