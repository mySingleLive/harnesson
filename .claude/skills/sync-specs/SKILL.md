...
- `design`：为每个节点生成对应的设计文档（Markdown 格式），保存到 `.harnesson/specs/draft/design/` 目录，节点 `design` 字段设为相对路径（如 `"design/ai-chat.md"`）。生成规则：
  - 自动检测节点类型：分析 `sourceFiles` 的文件扩展名和目录模式（详见 `references/design-doc-templates.md` 中的节点类型自动检测规则）
  - 按节点类型选择对应模板生成内容
  - 从代码中提取事实填充各章节，无内容的章节省略
  - 中间节点生成总体设计概述，叶子节点聚焦单个功能点
...
4. 按以下子步骤为每个节点生成设计文档：
   ...
   a. **检测节点类型**：根据 `references/design-doc-templates.md` 中的规则，分析 `sourceFiles` 自动判断节点类型（project/frontend/backend/fullstack）

   b. **生成设计文档**：按节点类型选择对应模板，从关联源文件中提取事实填充各章节。文件写入 `.harnesson/specs/draft/design/{node-path}.md`
     - 根节点 → `draft/design/project.md`
     - L2 节点 → `draft/design/{id}.md`
     - L3+ 节点 → `draft/design/{parent-path}/{id}.md`（路径结构与 nodes/ 目录一致）

   c. **设置 design 字段**：节点 JSON 中 `design` 字段设为相对路径字符串，如 `"design/ai-chat/message-input.md"`
