...
describe('TodoBar', () => {
  ...
  it('shows activeForm instead of subject for in_progress items', () => {
    render(<TodoBar todos={[{ id: '4', subject: 'Build feature', status: 'in_progress', activeForm: 'Building feature...' }]} />);
    expect(screen.getByText('Building feature...')).toBeInTheDocument();
    expect(screen.queryByText('Build feature')).not.toBeInTheDocument();
  });

  it('shows subject when activeForm is not set for in_progress item', () => {
    render(<TodoBar todos={[{ id: '5', subject: 'Build feature', status: 'in_progress' }]} />);
