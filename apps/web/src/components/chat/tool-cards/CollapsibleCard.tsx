...
export function CollapsibleCard({ icon, summary, preview, badge, children, isRunning }: CollapsibleCardProps) {
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (hasDetail && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); setOpen(!open); } }}
      className={`overflow-hidden rounded-md border border-harness-border bg-harness-bg ${hasDetail ? 'cursor-pointer' : ''}`}
    >
      <div className="flex items-center gap-2 bg-[#1a1a2e] px-2.5 py-1 text-[11px] text-left hover:bg-[#1f1f38] transition-colors">
        <span className={`transition-transform duration-200 text-gray-500 ${open ? 'rotate-90' : ''}`}>▸</span>
        {icon}
        {summary}
        {badge && <span className="ml-auto">{badge}</span>}
      </div>
      {!open && preview && <div className="px-2.5 pb-1 text-[11px] text-gray-600">{preview}</div>}
      {open && hasDetail && (
